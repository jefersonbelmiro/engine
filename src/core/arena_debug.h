...
static inline void arena_debug_restore(uint32_t id, size_t off)
{
  g_off[id] = off;
  while (g_track_count[id]) {
    arena_alloc_rec_t *r = &g_track[id][g_track_count[id] - 1];
    if (r->elem_size == 0) {
      if (r->offset >= off) { g_track_count[id]--; continue; }
      r->count = (uint32_t)(off - r->offset);
      r->elem_size = 1;
      break;
    }
    size_t end = r->offset + (size_t)r->count * r->elem_size;
    if (end <= off) break;
    if (r->offset < off) {
      size_t avail = off - r->offset;
      uint32_t n = (uint32_t)(avail / r->elem_size);
      if (n > 0) { r->count = n; break; }
    }
    g_track_count[id]--;
  }
}
